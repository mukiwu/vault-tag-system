...
import { TRIAL_NOTES, mapWithConcurrency, trialAvailable } from "./jev/client";
import type { TrialInfo } from "./jev/client";
...
export const useStore = create<Store>()((set, get) => {
  ...
  async function attach(root: FileSystemDirectoryHandle) {
    ...
    set({
      ...
      io,
      vaultName: root.name,
      excludedFolders: loadExcludedFolders(root.name),
    });
    await scan(root, io);
  }
  ...
  return {
    ...
    async judge() {
      ...
      // 試用只跑前幾篇。這裡擋是為了體驗，真正的額度由轉發層把關
      const targets = onTrial ? notes.slice(0, TRIAL_NOTES) : notes;
      ...
      try {
        const result = await runVault(targets, tags, {
          apiKey,
          model,
          // 試用額度靠 KV 計數，並行太多會小幅超扣
          concurrency: onTrial ? 1 : undefined,
          signal: controller.signal,
          onTrial: (info) => set({ trial: info }),
          onVerdict: (verdict) => {
            buffer.push(verdict);
            schedule();
          },
        ...
        set({
          phase: "ready",
          error:
            !stopped && result.failed > 0
              ? `有 ${result.failed} 篇筆記判定失敗`
              : null,
        });
      } catch (cause) {
        finish();
    ...
    pendingDiffs() {
      ...
      return notes
        .map((note, row) =>
          planNoteDiff(note, tags, rowCells(grid, row), thresholds),
        )
    ...
    async apply() {
      ...
      try {
        ...
        set({
          lastApply: { applied: result.applied, failures: result.failures },
        });
        await scan(root, io);
      } catch (cause) {
        set({ phase: "ready", error: message(cause) });
