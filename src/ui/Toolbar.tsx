...
export function Toolbar() {
  ...
  const busy = phase === 'judging' || phase === 'applying' || phase === 'scanning'
  ...
  return (
    <>
      {phase === 'judging' ? (
        ...
        <button className="primary" onClick={() => void judge()} disabled={busy || !grid}>
          開始判定
        </button>
      )}
      ...
      {counts && (
        <div className="counts mono">
          <span style={{ color: 'var(--auto)' }}>自動 {counts.add + counts['manual-add']}</span>
          <span style={{ color: 'var(--review)' }}>待審 {counts.pending}</span>
          <span style={{ color: 'var(--warn)' }}>廢棄 {counts['suggest-drop'] + counts.drop}</span>
        </div>
      )}
