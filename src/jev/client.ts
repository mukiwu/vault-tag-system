 ...
 * 呼叫 Jev 的薄層。
 *
 * Jev 的 CORS 白名單不收 localhost，瀏覽器沒辦法直接打它的 API，所以請求一律
 * 先送到自家的 /api/jev，由 Vite dev server 轉發出去。使用者填的金鑰放在
 * x-typesafe-key 這個自訂標頭裡，proxy 收到之後才換成正式的 Authorization，
 * 金鑰全程只在這台機器上流動。
 */

export const PROXY_ENDPOINT = '/api/jev'
...
export async function askJev(request: JevRequest, options: AskOptions = {}): Promise<JevResponse> {
  const {
