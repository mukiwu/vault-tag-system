...
新增標籤信心夠就自動做，移除既有標籤一律只提建議，要你點頭才會動。建議廢棄沒處理的話，寫入時會照原樣保留，待寫入區會提醒還剩幾個沒決定

三種調整粒度

- 整批：把整個審核區間一次採納或退回
- 整欄：點標籤名整欄採納，按住 alt 點整欄退回
- 單格：第一下照 AI 的提示走，待審核的是採納，建議廢棄的是移除；第二下反過來，第三下回到未決定

游標停在格子上時，矩陣底下的狀態列會寫出這格目前的狀態、寫入時的結果，以及點下去會變成什麼
